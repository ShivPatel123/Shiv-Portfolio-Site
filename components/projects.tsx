		...
		<section className="bg-gray-100 max-w-[42rem] border border-black/5 overflow-hidden sm:pr-8 relative sm:h-[20rem]">
			<div className="py-4 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%]">
				...
				<ul className="flex flex-wrap mt-4 gap-2">
					{tags.map((tag, index) => (
						<li
							className="bg-black/[0.7] px-3 py-1 text-[.7rem] uppercase tracking-wider text-white rounded-full"
							key={index}
						>
							{tag}
						</li>
			...
			<Image
				className="absolute top-8 -right-40 w-[28.25rem] roundedt-lg shadow-2xl"
				src={imageUrl}
				alt={title}
				quality={95}
			/>
